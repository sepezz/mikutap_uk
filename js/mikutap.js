$(function () {
  new MainManager().init();
});

let MainManager = function () {
    aidn.util.useDummyDiv();
    function a() {
      (G = aidn.window.width()),
        (A = aidn.window.height()),
        I && (I.resize(G, A), g && g.resize());
    }
    function e(n, a) {
      (a = R.length + C.length), 1 == b && (n += C.length);
      let e = Math.round((n / a) * 100) + "%";
      a <= 0 && (e = "0%"), $("#scene_loading hr").css("width", e);
    }
    function t() {
      1 == ++b ? R.init(t, e) : 2 == b && i();
    }
    function i() {
      (x = 1),
        $("#scene_loading hr").css("display", "none"),
        $("#scene_loading hr").css("width", 0),
        $("#scene_loading").stop().fadeOut(200, "linear"),
        p
          ? ($("#scene_loading").stop().css("display", "none"),
            $("#bt_back").stop().css("display", "none"),
            B && $("#bt_fs").stop().css("display", "none"),
            $("#scene_main .set").stop().css("display", "none"))
          : $("#scene_main").stop().fadeIn(200, "linear"),
        (m = aidn.___waContext.currentTime),
        g.start(),
        C.start();
    }
    function n(n) {
      (U = !U)
        ? ($("#bt_feedback a").text("Feedback: Open"),
          aidn.util.setCookie("fb", "on", 2592e3))
        : ($("#bt_feedback a").text("Feedback: Close"),
          aidn.util.setCookie("fb", "off", 2592e3)),
        n && n.preventDefault();
    }
    function o(n) {
      (T = !T)
        ? ($("#bt_backtrack a").text("Background music: On"),
          aidn.util.setCookie("bt", "on", 2592e3))
        : ($("#bt_backtrack a").text("Background music: off"),
          aidn.util.setCookie("bt", "off", 2592e3)),
        n && n.preventDefault();
    }
    function r() {
      C.update(),
        1 == x && --D < 0 && v(),
        (function () {
          if (!p) return;
          if (1 != x) return;
          let n = 1e3 * (aidn.___waContext.currentTime - m);
          if (l * s < n) {
            let a = Math.floor(n / s) + 1;
            h += a - l;
            let e = (l = a) * s - n;
            if (0 <= e) {
              let t = Math.random(),
                i = 1;
              192 <= h
                ? (h = 0)
                : 128 <= h
                ? (t < 0.7 && (i = 2), t < 0.5 && (i = 3))
                : 64 <= h
                ? (t < 0.35 && (i = 2), t < 0.2 && (i = 3), t < 0.02 && (i = 0))
                : 32 <= h
                ? (t < 0.35 && (i = 2), t < 0.24 && (i = 0))
                : 0 <= h && t < 0.4 && (i = 0);
              for (let o = 0; o < i; o++)
                (d = c[f]),
                  32 <= ++f && ((f = 0), aidn.util.shuffleArray(c)),
                  g.changeId(d, 0, !0);
            }
          }
        })(),
        I.render(z),
        window.requestAnimFrame(r);
    }
    this.init = function () {
      !(function () {
        aidn.window.addDummyDiv();
        try {
          aidn.adv.show();
        } catch (n) {}
        let n = 1;
        2 <= window.devicePixelRatio && (n = 2);
        ((I = PIXI.autoDetectRenderer(G, A, {
          backgroundColor: 16756655,
          antialias: !1,
          resolution: n,
        })).autoDensity = !0),
          document.getElementById("view").appendChild(I.view),
          (z = new PIXI.Container()),
          g.init(),
          a(),
          $("#scene_top").fadeIn(300),
          r();
      })();
    };
    for (
      let l = 0,
        s = 6e4 / 280,
        d = Math.floor(32 * Math.random()),
        h = 0,
        c = [],
        f = 0,
        u = 0;
      u < 32;
      u++
    )
      c[u] = u;
    function v() {
      p ||
        S ||
        ((S = !0),
        $("#bt_back").stop().fadeIn(200, "linear"),
        B && $("#bt_fs").stop().fadeIn(200, "linear"),
        $("#scene_main .set").stop().fadeIn(200, "linear"));
    }
    let p = !1;
    1 == aidn.util.getQuery().auto && (p = !0), aidn.util.needExpandArea(!0);
    let B = aidn.util.enabledFullscreen();
    B &&
      ($("#bt_fs").css("display", "block"),
      $("#bt_fs").click(function (n) {
        aidn.util.fullscreen();
      })),
      $("#bt_start a").click(function (n) {
        $("#scene_top").stop().fadeOut(200, "linear"),
          $("#scene_loading").stop().fadeIn(200, "linear"),
          2 == b ? i() : (new aidn.WebAudio().load(""), C.init(t, e));
        try {
          aidn.adv.hide();
        } catch (n) {}
        n.preventDefault();
      }),
      $("#bt_about a").click(function (n) {
        $("#about").stop().fadeIn(200, "linear"),
          $("#about_cover").stop().fadeIn(200, "linear"),
          n.preventDefault();
      }),
      $("#bt_close,#about_cover").click(function () {
        $("#about").stop().fadeOut(200, "linear"),
          $("#about_cover").stop().fadeOut(200, "linear");
      }),
      $("#bt_back").click(function () {
        switch (x) {
          case 1:
            x = 0;
            try {
              aidn.adv.show();
            } catch (n) {}
            g.end(),
              C.end(),
              $("#scene_top").stop().fadeIn(100, "linear"),
              $("#scene_loading").stop().fadeOut(100, "linear"),
              $("#scene_main").stop().fadeOut(100, "linear"),
              v();
            break;
          default:
            location.href = "https://github.com/HFIProgramming/mikutap/";
        }
      }),
      $("#bt_feedback a").click(n),
      $("#bt_backtrack a").click(o);
    let G,
      A,
      y = aidn.util.checkJapanese(),
      E = aidn.util.checkMobile(),
      w = "https://aidn.jp/mikutap/",
      M = "Mikutap";
    (M += y ? ",Hatsune Miku's 10th Anniversary" : ",Miku10th"),
      $("#bt_tw").click(function (n) {
        let a = document.title;
        aidn.social.shareTw(w, !0, a, "daniwell_aidn", M);
      }),
      $("#bt_fb").click(function (n) {
        aidn.social.shareFb(w, !0);
      }),
      $("#bt_gp").click(function (n) {
        aidn.social.shareGp(w, !0);
      });
    let m,
      I,
      z,
      b = 0,
      x = 0,
      C = new (function () {
        function t() {
          c && c();
        }
        function i(n, a) {
          f && f(n, a);
        }
        let r;
        (this.init = function (n, a) {
          (f = a),
            (function (n) {
              c = n;
              for (let a = [], e = 0; e < o; e++) a[e] = [e + ".mp3"];
              (r = new WebAudioManager()).load(
                "data/track/track.json",
                a,
                t,
                i
              );
            })(n);
        }),
          (this.update = function () {
            !(function () {
              if (l) {
                let n = 1e3 * (aidn.___waContext.currentTime - m);
                if (v * p < n) {
                  let a = (v = Math.floor(n / p) + 1) * p - n;
                  if (0 <= a) {
                    if (!T) return;
                    for (let e = (v - 1) % u, t = d.length, i = 0; i < t; i++) {
                      let o = d[i][e];
                      0 <= o && r.play(o, a / 1e3, s[o]);
                    }
                  }
                }
              }
            })();
          }),
          (this.start = function () {
            (l = !0), (v = 0);
          });
        this.end = function () {
          (l = !1), (v = 0);
        };
        let l = !1,
          o = 11;
        this.length = o;
        for (let s = [], n = 0; n < o; n++) s[n] = 1.2;
        s[1] *= 0.6;
        let d = [[0, 1, 2, 1], []],
          a = "";
        (a += "3443443443443434"),
          (a += "5665665665665656"),
          (a += "7887887887887878");
        let e = (a += "9119119119119191").length;
        for (n = 0; n < e; n++) {
          let h = parseInt(a.charAt(n));
          1 == h && (h = 10), (d[1][n] = h), 4 <= n && (d[0][n] = d[0][n % 4]);
        }
        let c,
          f,
          u = d[0].length,
          v = 0,
          p = 6e4 / 280;
      })(),
      R = new (function () {
        let o,
          r = -1,
          l = -1;
        function t() {
          c && c();
        }
        function i(n, a) {
          e && e(n, a);
        }
        this.init = function (n, a) {
          (e = a),
            (function (n) {
              c = n;
              for (let a = [], e = 0; e < s; e++) a[e] = [e + ".mp3"];
              (o = new WebAudioManager()).load("data/main/main.json", a, t, i);
            })(n);
        };
        this.play = function (n, a) {
          let e = 1e3 * (aidn.___waContext.currentTime + h[n] - m),
            t = Math.floor(e / f);
          t == r && 0 <= l && o.stop(l), (r = t), (l = n);
          let i = f - (e % f);
          o.play(n, i / 1e3, d[n]);
        };
        let s = 32;
        this.length = s;
        for (let d = [], h = [], n = 0; n < s; n++) (d[n] = 1), (h[n] = 0.05);
        for (
          h[6] = 0.08,
            h[20] = 0.1,
            h[23] = 0.1,
            d[1] = 1.3,
            d[2] = 1.6,
            d[3] = 1.35,
            d[5] = 1.7,
            d[9] = 0.8,
            d[17] = 0.8,
            d[22] = 0.9,
            d[25] = 0.7,
            d[29] = 1.2,
            n = 0;
          n < s;
          n++
        )
          d[n] *= 1.2;
        let c,
          e,
          f = 6e4 / 280;
      })(),
      g = new (function () {
        let s = function (n, a) {
            (this.id = n),
              (this.setPosition = function (n, a) {
                (r.position.x = e = n), (r.position.y = t = a);
              }),
              (this.setSize = function (n, a) {
                (i = n),
                  (o = a),
                  r.clear(),
                  r.beginFill(16777215),
                  (r.alpha = 0),
                  r.drawRect(0, 0, i, o);
              }),
              (this.play = function () {
                U &&
                  gsap.fromTo(
                    r,
                    0.5,
                    { alpha: 0.7 },
                    { alpha: 0, ease: Power0.easeNon }
                  );
              }),
              (this.hitcheck = function (n, a) {
                return e <= n && n < e + i && t <= a && a < t + o;
              });
            let e = 0,
              t = 0,
              i = 0,
              o = 0,
              r = new PIXI.Graphics();
            (r.interactive = !0), a.addChild(r);
          },
          n = function (n) {
            function a() {
              o.clear(), o.beginFill(16777215), o.drawRect(0, 0, G, A);
            }
            function e() {
              t.resize();
            }
            (this.resize = function () {
              o.clear(), o.beginFill(i), o.drawRect(0, 0, G, A);
            }),
              (this.flash = function () {
                r.setChildIndex(o, r.children.length - 1);
                for (let n = 0; n < 3; n += 2)
                  gsap.delayedCall(0.07 * n, a),
                    gsap.delayedCall(0.07 * (n + 1), e);
              }),
              (this.setColor = function (n, a) {
                (i = n),
                  (a = 0 <= a ? a : 0),
                  r.setChildIndex(o, a),
                  t.resize();
              });
            let t = this,
              i = 16777215,
              o = new PIXI.Graphics(),
              r = n;
            r.addChild(o);
          },
          d = function (n, a) {
            function e() {
              let n,
                a,
                e,
                t = 1.3 * d;
              if ((f.clear(), f.beginFill(0), f.moveTo(0, 0), 0 == o))
                for (let i = 0; i < c.rotation; i += 30)
                  (n = ((s * i + l) * Math.PI) / 180),
                    (a = Math.cos(n) * t),
                    (e = Math.sin(n) * t),
                    f.lineTo(a, e);
              else
                for (i = 360; c.rotation < i; i -= 30)
                  (n = ((s * i + l) * Math.PI) / 180),
                    (a = Math.cos(n) * t),
                    (e = Math.sin(n) * t),
                    f.lineTo(a, e);
              (n = ((s * c.rotation + l) * Math.PI) / 180),
                (a = Math.cos(n) * t),
                (e = Math.sin(n) * t),
                f.lineTo(a, e),
                f.lineTo(0, 0),
                f.endFill();
            }
            function t() {
              (o = 1),
                gsap.fromTo(
                  c,
                  0.9,
                  { rotation: 0 },
                  {
                    rotation: 360,
                    ease: Power1.easeOut,
                    onUpdate: e,
                    onComplete: i,
                  }
                );
            }
            function i() {
              r && r();
            }
            this.play = function (n, a) {
              (o = 0),
                (d = n),
                (r = a),
                (l = 360 * Math.random()),
                (s = 1),
                Math.random() < 0.5 && (s = -1),
                f.clear(),
                f.beginFill(0),
                f.moveTo(0, 0),
                f.lineTo(1, 1),
                f.endFill(),
                gsap.fromTo(
                  c,
                  0.6,
                  { rotation: 0 },
                  {
                    rotation: 360,
                    ease: Power1.easeOut,
                    onUpdate: e,
                    onComplete: t,
                  }
                );
            };
            let o,
              r,
              l,
              s,
              d,
              h = n,
              c = { rotation: 0 },
              f = new PIXI.Graphics();
            h.addChild(f), (a.mask = f);
          },
          r = function (n) {
            function h() {
              w.clear(), 0 == f ? w.lineStyle(v, u) : w.beginFill(u);
              for (let n = 0; n < p; n++) {
                let a = M["p" + n].x,
                  e = M["p" + n].y;
                0 == n ? w.moveTo(a, e) : w.lineTo(a, e);
              }
              (a = M.p0.x), (e = M.p0.y), w.lineTo(a, e);
            }
            function c() {
              (w.visible = !1), e && e();
            }
            this.play = function (n, a) {
              (f = n),
                (e = a),
                (function () {
                  y.setChildIndex(w, y.children.length - 1),
                    (w.visible = !0),
                    (w.x = G / 2),
                    (w.y = A / 2),
                    (u = L());
                  let n,
                    a = Math.min(G, A) * (0.32 * Math.random() + 0.16),
                    e = Math.floor(5 * Math.random()) + 3;
                  (p = e),
                    (v = 5 * Math.random() + 3),
                    w.clear(),
                    (w.rotation = 30 * Math.floor(6 * Math.random())),
                    (M = {}),
                    (n = 0 == f ? 3 : 2.5);
                  for (let t = 360 / p, i = 0; i < p; i++) {
                    let o = (i * t * Math.PI) / 180,
                      r = a * Math.cos(o),
                      l = a * Math.sin(o),
                      s = r + a * (Math.random() - 0.5) * n,
                      d = l + a * (Math.random() - 0.5) * n;
                    (M["p" + i] = { x: r, y: l }),
                      gsap.to(M["p" + i], 0.6, { x: s, y: d });
                  }
                  (M.progress = 0),
                    gsap.to(M, 0.8, {
                      progress: 1,
                      onUpdate: h,
                      onComplete: c,
                    });
                })();
            };
            let e,
              f,
              u,
              v,
              p,
              y = n,
              w = new PIXI.Graphics();
            y.addChild(w);
            let M = {};
          },
          t = function (n, a) {
            let f = function (n) {
              function i() {
                v.clear(),
                  v.lineStyle(d, h),
                  v.moveTo(s.x, s.y),
                  0 == u ? v.lineTo(r.x, r.y) : v.lineTo(l.x, l.y);
              }
              function o() {
                0 == u
                  ? ((u = 1),
                    (s = { x: r.x, y: r.y }),
                    gsap.to(s, f, {
                      x: l.x,
                      y: l.y,
                      ease: Power1.easeOut,
                      onUpdate: i,
                      onComplete: o,
                    }))
                  : (v.clear(), (v.visible = !1));
              }
              this.play = function (n, a, e, t) {
                return (
                  (v.visible = !0),
                  (u = 0),
                  (r = n),
                  (l = a),
                  (d = e),
                  (h = t),
                  (c = 0.2 * Math.random() + 0.2),
                  (f = 0.2 * Math.random() + 0.2),
                  (s = { x: r.x, y: r.y }),
                  gsap.to(s, c, {
                    x: l.x,
                    y: l.y,
                    ease: Power1.easeOut,
                    onUpdate: i,
                    onComplete: o,
                  }),
                  c + f
                );
              };
              let r,
                l,
                s,
                d,
                h,
                c,
                f,
                u,
                a = n,
                v = new PIXI.Graphics();
              a.addChild(v);
            };
            function u() {
              (y.visible = !1), 0 <= v.id && C[v.id].push(v), e && e();
            }
            this.play = function (n) {
              (e = n),
                (function () {
                  p.setChildIndex(y, p.children.length - 1),
                    (y.visible = !0),
                    (y.x = G / 2),
                    (y.y = A / 2),
                    (y.rotation =
                      0.5 * Math.PI * Math.floor(4 * Math.random()));
                  for (
                    let n,
                      a = Math.floor(7 * Math.random() + 2),
                      e = 0.8 * Math.min(G, A),
                      t = ((v.size = e) / a) * (0.4 * Math.random() + 0.7),
                      i = (e / a) * (0.4 * Math.random() + 0.1),
                      o = L(),
                      r = 0,
                      l = 0;
                    l <= a;
                    l++
                  ) {
                    let s = (l - 0.5 * a) * t,
                      d = { x: -e / 2, y: s },
                      h = { x: e / 2, y: s },
                      c = (n = w[l] ? w[l] : new f(y)).play(d, h, i, o);
                    r < c && (r = c), (w[l] = n);
                  }
                  gsap.delayedCall(r, u);
                })();
            };
            let v = this,
              p = n;
            this.id = a;
            let e,
              y = new PIXI.Container(),
              w = [];
            (this.size = 0), (this.container = y), p.addChild(y);
          };
        function l(n, a) {
          for (let e = M.length, t = 0; t < e; t++) {
            let i = M[t];
            if (i.hitcheck(n, a)) return u != i.id && i.play(), i.id;
          }
          return !1;
        }
        function a(n) {
          c(
            65 <= n.keyCode
              ? n.keyCode - 55
              : 48 <= n.keyCode
              ? n.keyCode - 48
              : n.keyCode
          );
        }
        function e(n) {
          c(-1);
        }
        function i(n) {
          y = !0;
          let a = aidn.event.getPos(n),
            e = l(a.x, a.y);
          if ((c(e), n.originalEvent && n.originalEvent.touches))
            for (let t = n.originalEvent.touches.length, i = 1; i < t; i++) {
              let o = n.originalEvent.touches[i];
              c((e = l(o.pageX, o.pageY)), 1);
            }
        }
        function o(n) {
          if (y) {
            let a = aidn.event.getPos(n);
            c(l(a.x, a.y), 0, !0);
          }
          n.preventDefault();
        }
        function h(n) {
          y && (c(-1), (y = !1));
        }
        function c(n, a, e) {
          let t, i;
          u != n &&
            (1 != a && (u = n),
            u < 0 ||
              (R.play(n % R.length, e),
              (D = 90),
              S &&
                ((S = !1),
                $("#bt_back").stop().fadeOut(200, "linear"),
                B && $("#bt_fs").stop().fadeOut(200, "linear"),
                $("#scene_main .set").stop().fadeOut(200, "linear")),
              --x <= 0 &&
                ((i = (t = Math.floor(I.length * Math.random())) + m.length),
                (C[i].length ? C[i].pop() : new I[t](b, i)).play(),
                (x = 12 * Math.random() + 6)),
              (t = n % m.length),
              (0 < C[t].length ? C[t].pop() : new m[t](b, t)).play()));
        }
        (this.resize = function () {
          if (w) {
            let n = 0,
              a = v,
              e = p;
            A < G && ((a = p), (e = v));
            for (let t = G / a, i = A / e, o = 0; o < e; o++)
              for (let r = 0; r < a; r++) {
                let l;
                M[n] ? (l = M[n]) : ((l = new s(n, f)), (M[n] = l)),
                  l.setPosition(t * r, i * o),
                  l.setSize(t, i),
                  n++;
              }
            T.resize();
          }
        }),
          (this.init = function () {
            (w = !0),
              (b = new PIXI.Container()),
              z.addChild(b),
              (f = new PIXI.Container()),
              z.addChild(f),
              (T = new n(b)).setColor(8965324, 0);
          }),
          (this.start = function () {
            E ||
              ($("#view").on("mousedown", i),
              $(window).on("mousemove", o),
              $(window).on("mouseup", h),
              $(window).on("keydown", a),
              $(window).on("keyup", e)),
              (E || window.TouchEvent) &&
                ($("#view").on("touchstart", i),
                $(window).on("touchmove", o),
                $(window).on("touchend", h)),
              $("#view").css("cursor", "pointer");
          }),
          (this.end = function () {
            E ||
              ($("#view").off("mousedown", i),
              $(window).off("mousemove", o),
              $(window).off("mouseup", h),
              $(window).off("keydown", a),
              $(window).off("keyup", e)),
              (E || window.TouchEvent) &&
                ($("#view").off("touchstart", i),
                $(window).off("touchmove", o),
                $(window).off("touchend", h)),
              $("#view").css("cursor", "auto");
          }),
          (this.changeId = function (n, a, e) {
            c(n, a, e);
          });
        let f,
          u = -1,
          v = 4,
          p = 8,
          y = !1,
          w = !1,
          M = [],
          m = [
            function (n, a) {
              let s = function (n) {
                function l() {
                  s.visible = !1;
                }
                this.play = function (n, a, e) {
                  (s.visible = !0), s.clear();
                  let t = G * Math.random(),
                    i = A * Math.random(),
                    o = Math.min(G, A) * (0.03 * Math.random() + 0.02);
                  s.lineStyle(3 * Math.random() + 3, e),
                    s.drawCircle(0, 0, o),
                    (s.x = n),
                    (s.y = a),
                    (s.scale.x = 0),
                    (s.scale.y = 0),
                    (s.rotation = Math.random() * Math.PI);
                  let r = 0.2 * Math.random() + 0.4;
                  return (
                    gsap.to(s, r, {
                      x: t,
                      y: i,
                      rotation: Math.random() * Math.PI,
                      ease: Power3.easeOut,
                      onComplete: l,
                    }),
                    gsap.to(s.scale, r, {
                      x: 1,
                      y: 1,
                      ease: Back.easeOut.config(1.7),
                    }),
                    r
                  );
                };
                let a = n,
                  s = new PIXI.Graphics();
                a.addChild(s);
              };
              function d() {
                (f.visible = !1), C[e.id].push(e);
              }
              this.play = function () {
                !(function () {
                  h.setChildIndex(f, h.children.length - 1), (f.visible = !0);
                  for (
                    let n = 5 * Math.random() + 7,
                      a = 0,
                      e = G / 2,
                      t = A / 2,
                      i = L(),
                      o = 0;
                    o < n;
                    o++
                  ) {
                    let r;
                    r = c[o] ? c[o] : new s(f);
                    let l = (c[o] = r).play(e, t, i);
                    a < l && (a = l);
                  }
                  gsap.delayedCall(a, d);
                })();
              };
              let e = this,
                h = n;
              this.id = a;
              let c = [],
                f = new PIXI.Container();
              h.addChild(f);
            },
            function (n, a) {
              let s = function (n) {
                function l() {
                  s.visible = !1;
                }
                this.play = function (n, a, e) {
                  (s.visible = !0), s.clear();
                  let t = G * Math.random(),
                    i = A * Math.random(),
                    o = Math.min(G, A) * (0.04 * Math.random() + 0.02);
                  s.beginFill(e),
                    s.drawRect(-o / 2, -o / 2, o, o),
                    (s.x = n),
                    (s.y = a),
                    (s.scale.x = 0),
                    (s.scale.y = 0),
                    (s.rotation = Math.random() * Math.PI);
                  let r = 0.2 * Math.random() + 0.4;
                  return (
                    gsap.to(s, r, {
                      x: t,
                      y: i,
                      rotation: Math.random() * Math.PI,
                      ease: Power3.easeOut,
                      onComplete: l,
                    }),
                    gsap.to(s.scale, r, {
                      x: 1,
                      y: 1,
                      ease: Back.easeOut.config(1.7),
                    }),
                    r
                  );
                };
                let a = n,
                  s = new PIXI.Graphics();
                a.addChild(s);
              };
              function d() {
                (f.visible = !1), C[e.id].push(e);
              }
              this.play = function () {
                !(function () {
                  h.setChildIndex(f, h.children.length - 1), (f.visible = !0);
                  for (
                    let n = 5 * Math.random() + 7,
                      a = 0,
                      e = G / 2,
                      t = A / 2,
                      i = L(),
                      o = 0;
                    o < n;
                    o++
                  ) {
                    let r;
                    r = c[o] ? c[o] : new s(f);
                    let l = (c[o] = r).play(e, t, i);
                    a < l && (a = l);
                  }
                  gsap.delayedCall(a, d);
                })();
              };
              let e = this,
                h = n;
              this.id = a;
              let c = [],
                f = new PIXI.Container();
              h.addChild(f);
            },
            function (n, a) {
              let h = function (n) {
                function r() {
                  h.beginFill(l),
                    h.drawCircle(0, 0, s),
                    (h.scale.x = 0),
                    (h.scale.y = 0),
                    gsap.to(h.scale, 0.7, {
                      x: 1,
                      y: 1,
                      ease: Elastic.easeOut.config(1, 0.3),
                      onComplete: a,
                    });
                }
                function a() {
                  gsap.to(h.scale, 0.4, {
                    x: 0,
                    y: 0,
                    ease: Power2.easeOut,
                    onComplete: e,
                    delay: 0.1,
                  });
                }
                function e() {
                  (h.visible = !1), d && d();
                }
                this.play = function (n, a, e, t, i, o) {
                  (h.visible = !0),
                    h.clear(),
                    (h.x = t),
                    (h.y = i),
                    (l = a),
                    (s = e),
                    (d = o),
                    gsap.delayedCall(n, r);
                };
                let l,
                  s,
                  d,
                  t = n,
                  h = new PIXI.Graphics();
                t.addChild(h);
              };
              function c() {
                (v.visible = !1), C[e.id].push(e);
              }
              this.play = function () {
                !(function () {
                  f.setChildIndex(v, f.children.length - 1),
                    (v.visible = !0),
                    (v.x = G / 2),
                    (v.y = A / 2),
                    (v.rotation = Math.random() * Math.PI * 2);
                  for (
                    let n = 10,
                      a = L(),
                      e = (Math.min(G, A) / 64) * (0.6 * Math.random() + 0.7),
                      t = 2,
                      i = 0;
                    i < 40;
                    i++
                  ) {
                    let o,
                      r = (25 * i * Math.PI) / 180,
                      l = n * Math.cos(r),
                      s = n * Math.sin(r);
                    (n += e),
                      (t += 0.22),
                      (o = u[i] ? u[i] : new h(v)),
                      (u[i] = o);
                    let d = null;
                    39 == i && (d = c), o.play(0.03 * i, a, t, l, s, d);
                  }
                })();
              };
              let e = this,
                f = n;
              this.id = a;
              let u = [],
                v = new PIXI.Container();
              f.addChild(v);
            },
            function (n, a) {
              function e() {
                C[t.id].push(t);
              }
              this.play = function () {
                o.play(0, e);
              };
              let t = this,
                i = n;
              this.id = a;
              let o = new r(i);
            },
            function (n, a) {
              function e() {
                C[t.id].push(t);
              }
              this.play = function () {
                o.play(1, e);
              };
              let t = this,
                i = n;
              this.id = a;
              let o = new r(i);
            },
            function (n, a) {
              function h() {
                (f.visible = !1), C[e.id].push(e);
              }
              this.play = function () {
                !(function () {
                  c.setChildIndex(f, c.children.length - 1),
                    (f.visible = !0),
                    (f.x = G / 2),
                    (f.y = A / 2);
                  let n = L(),
                    a = Math.min(G, A) * (0.28 * Math.random() + 0.2),
                    e = Math.floor(5 * Math.random()) + 3;
                  u.clear(),
                    u.lineStyle(7 * Math.random() + 4, n, 1),
                    (u.rotation = 30 * Math.floor(6 * Math.random()));
                  for (let t = 360 / e, i = 0; i <= e; i++) {
                    let o = (i * t * Math.PI) / 180,
                      r = a * Math.cos(o),
                      l = a * Math.sin(o);
                    0 == i ? u.moveTo(r, l) : u.lineTo(r, l);
                  }
                  let s = 0.8 * Math.random() + 0.4,
                    d = 0.8 * Math.random() + 0.4;
                  gsap.fromTo(
                    u.scale,
                    0.9,
                    { x: s, y: s },
                    { x: d, y: d, ease: Bounce.easeOut }
                  ),
                    v.play(a, h);
                })();
              };
              let e = this,
                c = n;
              this.id = a;
              let f = new PIXI.Container(),
                u = new PIXI.Graphics();
              c.addChild(f), f.addChild(u);
              let v = new d(f, u);
            },
            function (n, a) {
              let i = function (n) {
                function e() {
                  let n = Math.min(G, A),
                    a = n * (0.08 * Math.random() + 0.05);
                  l.lineStyle(4 * Math.random() + 4, L()),
                    l.drawRect(-a / 2, -a / 2, a, a),
                    (l.x = o + (n / 2) * (Math.random() - 0.5)),
                    (l.y = r + (n / 2) * (Math.random() - 0.5)),
                    (l.scale.x = 0),
                    (l.scale.y = 0),
                    (l.rotation = Math.random() * Math.PI),
                    gsap.to(l, 0.5, {
                      x: o,
                      y: r,
                      rotation: 0,
                      ease: Back.easeOut.config(1.7),
                      onComplete: t,
                    }),
                    gsap.to(l.scale, 0.5, {
                      x: 1,
                      y: 1,
                      ease: Back.easeOut.config(1.7),
                    });
                }
                function t() {
                  let n = Math.min(G, A),
                    a = o + (n / 2) * (Math.random() - 0.5),
                    e = r + (n / 2) * (Math.random() - 0.5);
                  gsap.to(l, 0.5, {
                    x: a,
                    y: e,
                    rotation: -Math.random() * Math.PI,
                    ease: Back.easeIn.config(1.7),
                    onComplete: i,
                    delay: 0.2,
                  }),
                    gsap.to(l.scale, 0.5, {
                      x: 0,
                      y: 0,
                      ease: Back.easeIn.config(1.7),
                      delay: 0.2,
                    });
                }
                function i() {
                  l.visible = !1;
                }
                this.play = function (n, a) {
                  (l.visible = !0),
                    l.clear(),
                    (o = G * Math.random()),
                    (r = A * Math.random()),
                    gsap.delayedCall(n, e);
                };
                let o,
                  r,
                  a = n,
                  l = new PIXI.Graphics();
                a.addChild(l);
              };
              function o() {
                (l.visible = !1), C[e.id].push(e);
              }
              this.play = function () {
                !(function () {
                  r.setChildIndex(l, r.children.length - 1), (l.visible = !0);
                  for (
                    let n = Math.floor(5 * Math.random() + 5), a = 0;
                    a < n;
                    a++
                  ) {
                    let e;
                    (e = s[a] ? s[a] : new i(l)), (s[a] = e);
                    let t = null;
                    a == n - 1 && (t = o), e.play(0.06 * a, t);
                  }
                })();
              };
              let e = this,
                r = n;
              this.id = a;
              let l = new PIXI.Container();
              r.addChild(l);
              let s = [];
            },
            function (n, a) {
              let i = function (n) {
                function e() {
                  let n = Math.min(G, A) * (0.05 * Math.random() + 0.014);
                  l.beginFill(L()),
                    l.drawCircle(0, 0, n),
                    (l.x = i),
                    (l.y = o),
                    (l.scale.x = 0),
                    (l.scale.y = 0),
                    gsap.to(l.scale, 0.5, {
                      x: 1,
                      y: 1,
                      ease: Elastic.easeOut.config(1, 0.3),
                      onComplete: a,
                    });
                }
                function a() {
                  gsap.to(l.scale, 0.5, {
                    x: 0,
                    y: 0,
                    ease: Back.easeIn.config(1.7),
                    onComplete: t,
                    delay: 0.2,
                  });
                }
                function t() {
                  l.visible = !1;
                }
                this.play = function (n, a) {
                  (l.visible = !0),
                    l.clear(),
                    (i = G * Math.random()),
                    (o = A * Math.random()),
                    gsap.delayedCall(n, e);
                };
                let i,
                  o,
                  r = n,
                  l = new PIXI.Graphics();
                r.addChild(l);
              };
              function o() {
                (l.visible = !1), C[e.id].push(e);
              }
              this.play = function () {
                !(function () {
                  r.setChildIndex(l, r.children.length - 1), (l.visible = !0);
                  for (
                    let n = Math.floor(5 * Math.random() + 5), a = 0;
                    a < n;
                    a++
                  ) {
                    let e;
                    (e = s[a] ? s[a] : new i(l)), (s[a] = e);
                    let t = null;
                    a == n - 1 && (t = o), e.play(0.06 * a, t);
                  }
                })();
              };
              let e = this,
                r = n;
              this.id = a;
              let l = new PIXI.Container();
              r.addChild(l);
              let s = [];
            },
            function (n, a) {
              function o() {
                (l.visible = !1), C[e.id].push(e);
              }
              this.play = function () {
                !(function () {
                  (l.visible = !0),
                    r.setChildIndex(l, r.children.length - 1),
                    (d.container.mask = s),
                    d.play(o);
                  let n = d.size / 2;
                  (s.x = G / 2),
                    (s.y = A / 2),
                    s.clear(),
                    s.beginFill(0),
                    s.drawCircle(0, 0, n);
                  let a =
                      ((45 * Math.PI) / 180) * Math.floor(2 * Math.random()),
                    e =
                      a +
                      ((45 * Math.PI) / 180) *
                        Math.floor(4 * Math.random() - 2),
                    t = 0.3 * Math.random() + 1,
                    i = 0.3 * -Math.random() + 1;
                  gsap.fromTo(
                    d.container,
                    0.6,
                    { rotation: a },
                    { rotation: e, ease: Power2.easeOut }
                  ),
                    gsap.fromTo(
                      d.container.scale,
                      0.6,
                      { x: t, y: t },
                      { x: i, y: i, ease: Back.easeOut.config(1.7) }
                    ),
                    gsap.fromTo(
                      s.scale,
                      0.6,
                      { x: t, y: t },
                      { x: i, y: i, ease: Back.easeOut.config(1.7) }
                    );
                })();
              };
              let e = this,
                r = n;
              this.id = a;
              let l = new PIXI.Container();
              r.addChild(l);
              let s = new PIXI.Graphics();
              l.addChild(s);
              let d = new t(l, -1);
            },
            function (n, a) {
              function d() {
                (c.visible = !1), C[e.id].push(e);
              }
              this.play = function () {
                !(function () {
                  h.setChildIndex(c, h.children.length - 1),
                    c.clear(),
                    (c.visible = !0),
                    c.lineStyle(5 * Math.random() + 3, L(), 1),
                    (c.x = G / 2),
                    (c.y = A / 2);
                  for (
                    let n = 0.6 * Math.min(G, A),
                      a = Math.floor(5 * Math.random()) + 3,
                      e = 360 / a,
                      t = ((0.5 * Math.max(G, A)) / n) * (1.6 + 0.6 / a),
                      i = 0;
                    i <= a;
                    i++
                  ) {
                    let o = (i * e * Math.PI) / 180,
                      r = n * Math.cos(o),
                      l = n * Math.sin(o);
                    0 == i ? c.moveTo(r, l) : c.lineTo(r, l);
                  }
                  let s = 0.3 * Math.random() + 0.6;
                  gsap.fromTo(
                    c.scale,
                    s,
                    { x: 0, y: 0 },
                    { x: t, y: t, onComplete: d, ease: Power2.easeOut }
                  ),
                    gsap.fromTo(
                      c,
                      s,
                      { rotation: Math.random() * Math.PI },
                      {
                        rotation: Math.random() * Math.PI,
                        ease: Power1.easeOut,
                      }
                    );
                })();
              };
              let e = this,
                h = n;
              this.id = a;
              let c = new PIXI.Graphics();
              h.addChild(c);
            },
            function (n, a) {
              function e() {
                (o.visible = !1), C[t.id].push(t);
              }
              this.play = function () {
                !(function () {
                  i.setChildIndex(o, i.children.length - 1),
                    (o.visible = !0),
                    (o.x = G / 2),
                    (o.y = A / 2);
                  let n = L(),
                    a = Math.min(G, A) * (0.25 * Math.random() + 0.1);
                  r.clear(),
                    r.beginFill(n),
                    r.drawCircle(0, 0, a),
                    l.play(a, e);
                })();
              };
              let t = this,
                i = n;
              this.id = a;
              let o = new PIXI.Container(),
                r = new PIXI.Graphics();
              i.addChild(o), o.addChild(r);
              let l = new d(o, r);
            },
            function (n, a) {
              let u = function (n) {
                function t() {
                  gsap.to(l.scale, 0.4, {
                    x: 0,
                    y: 0,
                    ease: Back.easeIn.config(2),
                    onComplete: a,
                    delay: 0.7,
                  }),
                    gsap.to(l, 0.4, {
                      rotation: Math.random() * Math.PI * 2,
                      ease: Back.easeIn.config(2),
                      delay: 0.7,
                    });
                }
                function a() {
                  (l.visibloe = !1), i && i();
                }
                (this.init = function (n, a, e, t) {
                  (_state = 0), (o = e), (r = t), (l.x = n), (l.y = a);
                }),
                  (this.play = function (n, a) {
                    (i = a),
                      l.clear(),
                      (l.visibloe = !0),
                      l.beginFill(r),
                      l.drawRect(0.5 * -o, 0.5 * -o, o, o),
                      gsap.fromTo(
                        l.scale,
                        0.3,
                        { x: 0, y: 0 },
                        {
                          x: 1,
                          y: 1,
                          ease: Back.easeOut.config(1.7),
                          onComplete: t,
                          delay: n,
                        }
                      ),
                      gsap.fromTo(
                        l,
                        0.7,
                        { rotation: Math.random() * Math.PI * 2 },
                        {
                          rotation: 0,
                          ease: Elastic.easeOut.config(1, 0.3),
                          delay: n,
                        }
                      );
                    let e = Math.random() * Math.PI;
                    gsap.fromTo(
                      y,
                      1,
                      { rotation: 0 },
                      { rotation: e, ease: Bounce.easeOut, delay: n }
                    );
                  });
                let i,
                  o,
                  r,
                  e = n,
                  l = new PIXI.Graphics();
                e.addChild(l);
              };
              function v() {
                (y.visible = !1), C[e.id].push(e);
              }
              this.play = function () {
                !(function () {
                  p.setChildIndex(y, p.children.length - 1),
                    (y.visible = !0),
                    (y.x = G / 2),
                    (y.y = A / 2);
                  let n = Math.floor(8 * Math.random() + 6),
                    a = Math.min(G, A) * (0.25 * Math.random() + 0.25),
                    e = 360 / n,
                    t = a * (0.15 * Math.random() + 0.05),
                    i = L(),
                    o = (Math.PI / 2) * Math.floor(4 * Math.random()),
                    r = 1;
                  Math.random() < 0.5 && (r = -1);
                  for (let l = 0; l < n; l++) {
                    let s,
                      d = ((r * e * l + o) * Math.PI) / 180,
                      h = a * Math.cos(d),
                      c = a * Math.sin(d);
                    (s = w[l] ? w[l] : new u(y)), (w[l] = s);
                    let f = null;
                    l == n - 1 && (f = v),
                      s.init(h, c, t, i),
                      s.play(0.05 * l, f);
                  }
                })();
              };
              let e = this,
                p = n;
              this.id = a;
              let y = new PIXI.Container(),
                w = [];
              p.addChild(y);
            },
            function (n, a) {
              let u = function (n) {
                function e() {
                  let n = 0.5 * G,
                    a = l.x + Math.random() * n - n / 2,
                    e = l.y + Math.random() * n - n / 2;
                  gsap.to(l.scale, 0.3, {
                    x: 0,
                    y: 0,
                    ease: Power1.easeOut,
                    onComplete: t,
                    delay: 0.5,
                  }),
                    gsap.to(l, 0.3, {
                      x: a,
                      y: e,
                      ease: Power2.easeOut,
                      delay: 0.5,
                    });
                }
                function t() {
                  (l.visibloe = !1), i && i();
                }
                (this.init = function (n, a, e, t) {
                  (_state = 0), (o = e), (r = t), (l.x = n), (l.y = a);
                }),
                  (this.play = function (n, a) {
                    (i = a),
                      l.clear(),
                      (l.visibloe = !0),
                      l.beginFill(r),
                      l.drawCircle(0, 0, 0.5 * o),
                      gsap.fromTo(
                        l.scale,
                        0.3,
                        { x: 0, y: 0 },
                        {
                          x: 1,
                          y: 1,
                          ease: Back.easeOut.config(1.7),
                          onComplete: e,
                          delay: n,
                        }
                      );
                  });
                let i,
                  o,
                  r,
                  a = n,
                  l = new PIXI.Graphics();
                a.addChild(l);
              };
              function v() {
                (y.visible = !1), C[e.id].push(e);
              }
              this.play = function () {
                !(function () {
                  p.setChildIndex(y, p.children.length - 1),
                    (y.visible = !0),
                    (y.x = G / 2),
                    (y.y = A / 2);
                  let n = Math.floor(8 * Math.random() + 6),
                    a = Math.min(G, A) * (0.2 * Math.random() + 0.25),
                    e = 360 / n,
                    t = a * (0.2 * Math.random() + 0.05),
                    i = L(),
                    o = (Math.PI / 2) * Math.floor(4 * Math.random()),
                    r = 1;
                  Math.random() < 0.5 && (r = -1);
                  for (let l = 0; l < n; l++) {
                    let s,
                      d = ((r * e * l + o) * Math.PI) / 180,
                      h = a * Math.cos(d),
                      c = a * Math.sin(d);
                    (s = w[l] ? w[l] : new u(y)), (w[l] = s);
                    let f = null;
                    l == n - 1 && (f = v),
                      s.init(h, c, t, i),
                      s.play(0.05 * l, f);
                  }
                })();
              };
              let e = this,
                p = n;
              this.id = a;
              let y = new PIXI.Container(),
                w = [];
              p.addChild(y);
            },
            function (n, a) {
              function i() {
                (r.visible = !1), C[e.id].push(e);
              }
              this.play = function () {
                !(function () {
                  (r.visible = !0),
                    o.setChildIndex(r, o.children.length - 1),
                    (r.x = 0.2 * G + 0.6 * G * Math.random()),
                    (r.y = 0.2 * A + 0.6 * A * Math.random());
                  let n,
                    a = Math.min(G, A) * (0.7 + 0.2 * Math.random()),
                    e = (a / 10) * (0.5 + 0.8 * Math.random()),
                    t = L();
                  l.clear(),
                    l.beginFill(t),
                    l.drawRect(0, -e / 2, a, e),
                    s.clear(),
                    s.beginFill(t),
                    s.drawRect(-e / 2, 0, e, a),
                    (l.y = 0),
                    (l.x = -a / 2),
                    (s.x = 0),
                    (s.y = -a / 2),
                    (r.rotation = (45 * Math.PI) / 180),
                    (l.scale.x = 0),
                    (s.scale.y = 0),
                    (n =
                      Math.random() < 0.5
                        ? (-135 * Math.PI) / 180
                        : (215 * Math.PI) / 180),
                    new TimelineLite()
                      .to(l.scale, 0.4, { x: 1, ease: Power2.easeOut })
                      .to(s.scale, 0.4, { y: 1, ease: Power2.easeOut }, 0.1)
                      .to(
                        r,
                        0.6,
                        { rotation: n, ease: Back.easeOut.config(1.7) },
                        0
                      )
                      .to(l.scale, 0.3, { x: 0, ease: Power2.easeOut })
                      .to(
                        s.scale,
                        0.3,
                        { y: 0, ease: Power2.easeOut, onComplete: i },
                        0.6
                      );
                })();
              };
              let e = this,
                o = n;
              this.id = a;
              let r = new PIXI.Container(),
                l = new PIXI.Graphics(),
                s = new PIXI.Graphics();
              o.addChild(r), r.addChild(l), r.addChild(s);
            },
            function (n, a) {
              function o() {
                if (r < ++c)
                  switch (l) {
                    case 0:
                      l = 1;
                      let n = h[0];
                      (f.x = n.x), (f.y = n.y), (c = 0), o();
                      break;
                    case 1:
                      l = 2;
                  }
                else
                  gsap.to(f, 0.1, {
                    x: h[c].x,
                    y: h[c].y,
                    onComplete: o,
                    onUpdate: e,
                    ease: Power1.easeOut,
                  });
              }
              function e() {
                switch ((u.clear(), u.lineStyle(s, d, 1), l)) {
                  case 0:
                    u.moveTo(h[0].x, h[0].y);
                    for (let n = 1; n < c; n++) u.lineTo(h[n].x, h[n].y);
                    u.lineTo(f.x, f.y);
                    break;
                  case 1:
                    for (u.moveTo(f.x, f.y), n = c; n <= r; n++)
                      u.lineTo(h[n].x, h[n].y);
                }
              }
              this.play = function () {
                !(function () {
                  u.clear(),
                    (u.visible = !0),
                    Math.random() < 0.5
                      ? ((u.x = 0), (u.y = 0), (u.rotation = 0))
                      : ((u.x = G), (u.y = A), (u.rotation = Math.PI)),
                    (c = l = 0),
                    (r = Math.floor(3 * Math.random()) + 3),
                    (s = 20 * Math.random() + 2),
                    (d = L());
                  let n,
                    a = Math.random() < 0.5;
                  n = a ? G / r : A / r;
                  for (let e = 0; e <= r; e++) {
                    let t;
                    a
                      ? ((t = { x: e * n, y: A * Math.random() }),
                        0 == e && (t.x -= 10),
                        e == r && (t.x += 10))
                      : ((t = { y: e * n, x: G * Math.random() }),
                        0 == e && (t.y -= 10),
                        e == r && (t.y += 10)),
                      (h[e] = t);
                  }
                  let i = h[0];
                  (f.x = i.x), (f.y = i.y), o();
                })();
              };
              let t = n;
              this.id = a;
              let r,
                l,
                s,
                d,
                h = [],
                c = 0,
                f = { x: 0, y: 0 },
                u = new PIXI.Graphics();
              t.addChild(u);
            },
            t,
          ],
          I = [
            function (n, a) {
              function d() {
                y.clear(),
                  y.beginFill(c),
                  y.moveTo(u.pos.b1.x, u.pos.b1.y),
                  y.lineTo(u.pos.b0.x, u.pos.b0.y);
                for (let n = 0; u.pos["p" + n]; n++) {
                  let a = u.pos["p" + n];
                  y.lineTo(a.x, a.y);
                }
                y.endFill();
              }
              function h() {
                F == w && T.setColor(c, p - 1),
                  (y.visible = !1),
                  C[u.id].push(u);
              }
              this.play = function () {
                !(function () {
                  F = w;
                  let n = X();
                  (c = P[n]),
                    $("#about").css("background-color", "#" + c.toString(16)),
                    Math.random() < 0.3 && T.flash(p),
                    (O = n),
                    y.clear(),
                    (y.visible = !0),
                    (p = v.children.length - 1 - Math.floor(2 * Math.random())),
                    v.setChildIndex(y, p);
                  let a = Math.random() < 0.5,
                    e = Math.floor(4 * Math.random()) + 1;
                  u.pos = {};
                  let t = 0;
                  a
                    ? ((t = A / e),
                      (u.pos.b0 = { x: 0, y: 0 }),
                      (u.pos.b1 = { x: 0, y: A }))
                    : ((t = G / e),
                      (u.pos.b0 = { x: 0, y: 0 }),
                      (u.pos.b1 = { x: G, y: 0 })),
                    Math.random() < 0.5
                      ? ((y.rotation = 0), (y.x = 0), (y.y = 0))
                      : ((y.rotation = Math.PI), (y.x = G), (y.y = A));
                  for (let i = (f = 0); i <= e; i++) {
                    let o = { x: 0, y: 0 },
                      r = 0;
                    0 != i && i != e && (r = (t / 4) * Math.random() - t / 8),
                      a ? (o.y = t * i + r) : (o.x = t * i + r),
                      (u.pos["p" + i] = o);
                    let l,
                      s = 0.4 * Math.random() + 0.3;
                    (f = 2),
                      (l = a ? { x: G } : { y: A }),
                      gsap.to(u.pos["p" + i], s, l);
                  }
                  (u.progress = 0),
                    gsap.to(u, f, {
                      progress: 1,
                      ease: Power0.easeNone,
                      onUpdate: d,
                      onComplete: h,
                    });
                })();
              };
              let c,
                f,
                u = this,
                v = n;
              (this.id = a), (this.progress = 0), (this.pos = {});
              let p = 0,
                y = new PIXI.Graphics();
              v.addChild(y);
              let w = Math.floor(aidn.util.getTime());
            },
          ];
        aidn.util.shuffleArray(m);
        for (
          let b, x = 16 * Math.random(), C = [], g = 0;
          g < m.length + I.length;
          g++
        )
          C[g] = [];
        let T,
          P = [
            13430510, 8965324, 9099756, 961181, 1089457, 34969, 13934238,
            16110792, 15488645, 16531063, 5853015, 3222317,
          ],
          k = [
            13430510, 8965324, 9099756, 961181, 1089457, 34969, 13934238,
            16110792, 15488645, 16531063, 5853015, 3222317,
          ],
          _ = P.length,
          O = 0;
        function L() {
          let n = Math.random();
          return n < 0.03 ? 4473924 : n < 0.18 ? 16777215 : k[X()];
        }
        function X() {
          for (let n = 0; n < 10; n++) {
            let a = Math.floor(_ * Math.random());
            if (2 < Math.abs(O - a)) break;
          }
          return a;
        }
        let F = 0;
      })(),
      S = !1,
      D = 0,
      U = "off" == aidn.util.getCookie("fb"),
      T = "off" == aidn.util.getCookie("bt");
    n(),
      o(),
      aidn.util.webaudio
        ? ($("#ng").css("display", "none"),
          $(".ok").css("display", "block"),
          E && $("#scene_main .attention").html("TOUCH &amp; SWIPE!"),
          y || $("#scene_top .attention").text("* Raise the volume and enjoy!"))
        : ($("#ng").css("display", "block"),
          $(".ok").css("display", "none"),
          y ||
            $("#ng .atten").html(
              "Sorry,<br>your device or browser doesn't support this site."
            )),
      (PIXI.utils._saidHello = !0),
      aidn.window.resize(a);
  };

let WebAudioManager = function () {
  function i() {
    if ((a++, (h.now = a), s && s(a, o), o <= a)) l && l();
    else {
      let n = new aidn.WebAudio();
      n.load(d[r[a]], i), (t[a] = n);
    }
  }
  (this.load = function (n, a, e, t) {
    (l = e),
      (s = t),
      (o = (r = a).length),
      (h.length = o),
      $.getJSON(n, function (n) {
        (d = n), i();
      });
  }),
    (this.play = function (n, a, e) {
      0 <= e || (e = 1), n < o && t[n].play(0, !1, null, 0, e, a);
    }),
    (this.stop = function (n) {
      n < o && t[n].stop();
    });
  let o,
    r,
    l,
    s,
    d,
    h = this,
    a = -1,
    t = [];
  (this.length = 0), (this.now = 0);
};